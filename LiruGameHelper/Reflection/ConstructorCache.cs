...
namespace LiruGameHelper.Reflection
{
    public class ConstructorCache<T>
    {
        ...
        public ConstructorCache(Assembly assembly, string defaultPath = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(defaultPath)) RegisterNamespace(assembly, defaultPath);
        }
        ...
        public void RegisterNamespace(Assembly assembly, string namespacePath)
        {
            ...
            if (string.IsNullOrWhiteSpace(namespacePath)) throw new ArgumentException("Element namespace cannot be null or empty.", nameof(namespacePath));
            ...
            foreach (Type type in assembly.GetTypes())
            {
                ...
                if (type.Namespace.Contains(namespacePath)) RegisterType(type);
            }
        }
        ...
        public bool RegisterType(Type type)
        {
            ...
            if (!type.IsClass || type.IsAbstract || !typeof(T).IsAssignableFrom(type)) return false;
        ...
        public ConstructorInfo GetConstructor(Type type)
        {
            ...
            if (!constructorsByType.TryGetValue(type, out ConstructorInfo constructor)) throw new Exception($"The type with the name \"{type.Name}\" has no constructor registered in this cache.");
        ...
        public Type GetTypeFromName(string name)
        {
            ...
            if (!typesByName.TryGetValue(name, out Type componentType)) throw new Exception($"No component with the name \"{name}\" is registered.");
        ...
        public T1 CreateInstance<T1>(string name, params object[] parameters) where T1 : T => (T1)CreateInstance(name, parameters);
        #endregion
    }
}
