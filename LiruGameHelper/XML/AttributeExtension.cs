...
public static class AttributeExtension
{
    ...
    public static void AddAttribute(this XmlNode node, string name, object value)
    {
        if (node.OwnerDocument == null || node.Attributes == null)
            throw new ArgumentException("Node was missing owner document or attributes!", nameof(node));
        XmlAttribute attribute = node.OwnerDocument.CreateAttribute(name);
        attribute.Value = value.ToString();
        node.Attributes.Append(attribute);
    }

    public static void AddAttributeList<T>(this XmlNode node, string name, IReadOnlyCollection<T> values, char separator = ',') where T : notnull
    {
        if (node.OwnerDocument == null || node.Attributes == null)
            throw new ArgumentException("Node was missing owner document or attributes!", nameof(node));
        XmlAttribute attribute = node.OwnerDocument.CreateAttribute(name);
    ...
    {
        bool parsed = node.TryParseAttributeValue(attributeName, parser, out T? parsedValue);
        output = parsed ? parsedValue! : defaultTo;
        return parsed;
    }

    public static string GetAttributeValue(this XmlNode node, string attributeName)
        => (node == null || !node.GetAttributeValue(attributeName, out string value)) 
            ? throw new FormatException($"{node?.Name}'s {attributeName} could not be parsed.")
            : value.Replace(@"\n", Environment.NewLine);
